...
# First run all combinations with cf252source.py
echo "Running simulations with cf252source.py..."
for density_offset in $(seq -0.5 0.1 0.1); do
    ...
    for hdpe_density_offset in $(seq -0.5 0.1 0.1); do
        ...
        for radius in $(seq 0.1 0.1 1.0); do
            ...
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python cf252source.py && \
            ...
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python plot_track.py "cf252source.py" "$fuel_density" "$coolant_density" "$radius"
        done
    done
done

# Now run all combinations with cf252sourceair.py
echo "Running simulations with cf252sourceair.py..."
for density_offset in $(seq -0.5 0.1 0.1); do
    fuel_density=$(echo "$ISOTOPES_DENSITY + $density_offset" | bc -l)
    for air_density_offset in $(seq -0.5 0.1 0.1); do
        ...
        for radius in $(seq 0.1 0.1 1.0); do
            ...
            # Run the OpenMC simulation
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python cf252sourceair.py && \
            
            # Run the OpenMC tally command
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif openmc -t && \
            
            # Run the analysis script
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python plot_track.py "cf252sourceair.py" "$fuel_density" "$coolant_density" "$radius"
