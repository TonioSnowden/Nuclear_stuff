...
ISOTOPES_DENSITY=19.1  # Base density value for isotopes
...
# First run all combinations with u238source.py
echo "Running simulations with u238source.py..."
for density_offset in $(seq -0.5 0.1 0.1); do
    ...
    for hdpe_density_offset in $(seq -0.5 0.1 0.1); do
        ...
        for radius in $(seq 0.1 0.1 1.0); do
            ...
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python u238source.py && \
            ...
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python plot_track.py "u238source.py" "$fuel_density" "$coolant_density" "$radius"
        done
    done
done

# Now run all combinations with u238sourceair.py
echo "Running simulations with u238sourceair.py..."
for density_offset in $(seq -0.5 0.1 0.1); do
    fuel_density=$(echo "$ISOTOPES_DENSITY + $density_offset" | bc -l)
    for air_density_offset in $(seq -0.5 0.1 0.1); do
        ...
        for radius in $(seq 0.1 0.1 1.0); do
            ...
            # Run the OpenMC simulation
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python u238sourceair.py && \
            
            # Run the OpenMC tally command
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif openmc -t && \
            
            # Run the analysis script
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python plot_track.py "u238sourceair.py" "$fuel_density" "$coolant_density" "$radius"
