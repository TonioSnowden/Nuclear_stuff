...
# First run all combinations with u235source.py
echo "Running simulations with u235source.py..."
for density_offset in $(seq -0.5 0.1 0.1); do
    ...
    for hdpe_density_offset in $(seq -0.5 0.1 0.1); do
        ...
        for radius in $(seq 0.1 0.1 1.0); do
            ...
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python u235source.py && \
            ...
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python plot_track.py "u235source.py" "$fuel_density" "$coolant_density" "$radius"
        done
    done
done

# Now run all combinations with u235sourceair.py
echo "Running simulations with u235sourceair.py..."
for density_offset in $(seq -0.5 0.1 0.1); do
    fuel_density=$(echo "$ISOTOPES_DENSITY + $density_offset" | bc -l)
    for air_density_offset in $(seq -0.5 0.1 0.1); do
        ...
        for radius in $(seq 0.1 0.1 1.0); do
            ...
            # Run the OpenMC simulation
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python u235sourceair.py && \
            
            # Run the OpenMC tally command
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif openmc -t && \
            
            # Run the analysis script
            apptainer exec --bind /global/scratch/users/toniooppi/nuclear_data:/nuclear_data /global/scratch/users/toniooppi/openmc_latest.sif python plot_track.py "u235sourceair.py" "$fuel_density" "$coolant_density" "$radius"
